...
function setAccounsMap(array) {
    ...
    if(mapAccounts.length > 0)
        setCurrenAccoundKind(mapAccounts[0].Number);
}
...
function setCurrenAccoundKind(account){
    ...
    if(mapAccounts){
        ...
        if(obj){
            ...
            if(Currencies)
                EdocsApi.setAttributeValue({code: 'CounterpartyCurrency', value: Currencies.id, text: Currencies.value, itemCode: Currencies.code});
...
function onChangeContractExtensioInfo(){
    ...
    if(ProlongationEnd){
        ProlongationEnd.value = EdocsApi.getAttributeValue('ContractExtensioInfo')?.value?.pop()?.find(x=>x.code == 'ContractExtensioDate')?.value;
...
function actualOrganization(data){
    ...
    //EdocsApi.setAttributeValue({code: 'OrgTaxNumber', value: data.taxId, text: null});
    EdocsApi.setAttributeValue({code: 'OrgINN', value: data.taxId, text: null});
    //EdocsApi.setAttributeValue({code: 'OrgCurrency', value: null, text: null});
...
function onCardInitialize() {
    ...
    if(new Date('02/12/2023') > new Date(CurrentDocument.created))
     EdocsApi.setControlProperties({code: 'ContractAmountChoice', hidden: false, disabled: false, required: false});
...
function onBeforeCardSave() {
    ...
    if (planAmountTitle.value && planAmountTitle.value.length) {
        var requiredRow = planAmountTitle.value.map(t => t.filter(x => x.code == 'PlanAmountStartDate' && !x.value || x.code == 'PlanAmountEndDate' && !x.value || x.code == 'PlanAmount' && !x.value))
            .flat();
//EdocsApi.message('заповніть будь ласка поля: ');
    ...
    if (addAgrAddingChanges.value == 'true') {
        ...
        if (!len?.length)
            throw `Заповніть обов'язкові поля блоку "ПРОЛОНГАЦІЯ ДОГОВОРУ"`
    }
}
...
function setStoragePeriod() {
    ...
    if (contarctEndDate.value && !generalStorageTerm.value.length && documentTypeeDocs.value) {
        ...
        generalStorageTerm.value[0]?.forEach(x => {if (x.code == 'StorageTerm') x.value = null});
    }
...
function setContractStatus() {
    ...
    var dates = [
        EdocsApi.getAttributeValue('ContarctEndDate').value,
        EdocsApi.getAttributeValue('ContractExtensioDate').value,
        EdocsApi.getAttributeValue('ContractExtensioDate1').value
    ];
    //debugger;
    if (!hasInit(dates)) {
...
function onTaskExecuteSendOutDoc(routeStage) {
    ...
    var methodData = {
        extSysDocId:
        CurrentDocument.id, ExtSysDocVersion: CurrentDocument.version
    };
...
function setDataForESIGN() {
    ...
    name += ' №' + (regNumber ? regNumber : CurrentDocument.id) + (!regDate ? '' : (' від ' + moment(regDate)
        .format('DD.MM.YYYY')));
    var doc = {
        SystemCode: "mhp",
        DocumentName: name,
...
function onRouteStart() {
    ...
    */
    ...
    if(EdocsApi.getAttributeValue('CounterpartyStatus').value != "Не підлягає"){
        if(!auditDateUntil)
            throw 'По даному контрагенту не вказано дату НПК';    
...
////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(!ContarctEndDate)
        throw 'По даному контрагенту не вказано скрок дії договору';
        if(currentDate > (new Date(ContarctEndDate).setHours(0, 0, 0, 0)))
            throw 'По даному контрагенту строк дії договору закінчився';
...
function clearTables() {
    var planAmountTitle = EdocsApi.getAttributeValue('PlanAmountTitle');
    if (planAmountTitle && planAmountTitle.value && planAmountTitle.value.length) {
        planAmountTitle.value.forEach(t => t.forEach(x => {
            if (x.code == 'PlanAmountStartDate' || x.code == 'PlanAmountEndDate' || x.code == 'PlanAmount') {
                x.value = null;
            }
        }));
...
function setSigner() {
    ...
    if(Approval && !(Approval.executionState == 'complete')){
        ...
        if (!(managerDepartment.text && organization.value) || !organization.value || !responsibleDirector.value) {
            ...
            var array = EdocsApi.getDictionaryData('Directors', null, [{attributeCode: 'Initiator', value: managerDepartment.text},
        ...
        var directorBUM2 = EdocsApi.getDictionaryData('Directors', null, [{attributeCode: 'ResponsibleDirector', value: responsibleDirector?.text},
        {attributeCode: 'Initiator', value: managerDepartment?.text},
        {attributeCode: 'Title', value: organization?.value},
        {attributeCode: 'Director1', value: director1?.value},
        {attributeCode: 'FinDirector', value: director1?.value}])?.[0]?.id;
        EdocsApi.setAttributeValue({code:'BUMDirector2', value: (directorBUM2? 1: 0).toString(), text: null});

            //debugger;
        var directorBUM = EdocsApi.getDictionaryData('Directors', null, [/*{attributeCode: 'ResponsibleDirector', value: responsibleDirector?.text},*/
        {attributeCode: 'Initiator', value: managerDepartment?.text},
        {attributeCode: 'Title', value: organization?.value},
        {attributeCode: 'FinDirector', value: director1?.value}])?.[0]?.id;
        EdocsApi.setAttributeValue({code:'DirectorBUM', value: (directorBUM? 1: 0).toString(), text: null});

        var director2FIn = EdocsApi.getDictionaryData('Directors', null, [/*{attributeCode: 'ResponsibleDirector', value: responsibleDirector?.text},*/
        {attributeCode: 'Initiator', value: managerDepartment?.text},
        {attributeCode: 'Title', value: organization?.value},
        {attributeCode: 'FinDirector', value: director2?.value}])?.[0]?.id;
